...
Sibling [`messaging`](../008-messaging/spec.md) feature already shipped the **outbound** events
...
> - Tier-1 SPI (`AdapterModule` + ServiceLoader) lives in [`adapter-modules`](../002-adapter-modules/spec.md).
    ...
    > [`messaging`](../008-messaging/spec.md) `events` surface.
> - Existing `Nx-Server-Id` + `Nx-Message-Type` + `Nx-Correlation-Id` header contracts from
    > [`per-server-sync`](../007-per-server-sync/spec.md) and [`messaging`](../008-messaging/spec.md)
...
- Sibling feature (topic delivery contract):
  [`docs/specs/001-adapter-bootstrap/spec.md`](../001-adapter-bootstrap/spec.md)
- Sibling feature (`Nx-Server-Id` header stamping):
  [`docs/specs/007-per-server-sync/spec.md`](../007-per-server-sync/spec.md)
- Companion document:
  [`docs/specs/009-commands/guide.md`](./guide.md) — handler-author walkthrough
