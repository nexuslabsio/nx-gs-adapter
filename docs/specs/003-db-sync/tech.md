...
- `nx-gs-db-sync-core/src/main/java/app/l2nx/gs/db/sync/`
    - `DbSyncModule.java` — implements `app.l2nx.gs.adapter.api.spi.AdapterModule`;
      ...
      (defined in [`cdc-engine`](../005-cdc-engine/tech.md)) with the resolved
      `DbSchemaProvider`'s mappings.
    - `engine/` — CDC algorithm; structure documented in
      [`cdc-engine/tech.md`](../005-cdc-engine/tech.md). Engine code physically ships in
...
- **CDC engine** — `CdcEngine`, `EntitySyncTask`, `Phase1Hasher`,
  ...
  [`cdc-engine/tech.md`](../005-cdc-engine/tech.md). The module assembles the
...
Documented in [`cdc-engine/tech.md`](../005-cdc-engine/tech.md) data flows. Initial sync is
...
- **Adapter-side persistence: per-entity snapshot files on local disk** —
  ...
  offline. See [`snapshot-persistence`](../012-snapshot-persistence/spec.md)
...
- **`cdc-engine` feature** — engine code physically lives in `nx-gs-db-sync-core` JAR;
  ...
  [`cdc-engine/tech.md`](../005-cdc-engine/tech.md). `DbSyncModule` instantiates `CdcEngine`
  in Phase 2 `start()`.
...
- **Engine-algorithm decisions (CRC32 sufficiency, fastutil RAM model, window-scoped
  ...
  [`cdc-engine/tech.md`](../005-cdc-engine/tech.md) Decisions** to keep db-sync's design
...
- **Engine algorithm extensions** (new strategy, alternate hash function, composite/
  ...
  [`cdc-engine/tech.md`](../005-cdc-engine/tech.md) Extension points.
