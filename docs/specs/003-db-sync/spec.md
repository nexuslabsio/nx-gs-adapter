...
> - Tier-3 SPI (`JdbcConnectionSource`) lives in
    > [`jdbc-connection-source`](../004-jdbc-connection-source/spec.md) (interface in
...
- [wip] R5. **[Phase 2]** `EntityMapping<T>` interface MUST describe ONE synced
  ...
  zero-or-more **child sources** (each with FK back to primary's PK).
    - `String entityName()` — domain identifier in singular form: `"clan"`,
      ...
      R16](../001-adapter-bootstrap/spec.md)). Surfaced through heartbeat as
      `EntityStats.name`.
  ...
  **Notes:**
    ...
    - Kafka topic names are NOT declared by the schema provider either; they
      ...
      `entityName()`. See [`adapter-bootstrap` R16](../001-adapter-bootstrap/spec.md)
...
- [done] R16. **[Phase 2]** `DbSyncModule.onConnect` MUST consult
  ...
  [`adapter-bootstrap` R16](../001-adapter-bootstrap/spec.md)) before instantiating the
  CDC engine. Behavior:
...
- [resolved: Topic naming — per-entity topics, names supplied by the platform via
  ...
  [`adapter-bootstrap` R16](../001-adapter-bootstrap/spec.md)). Adapter does not
...
- Sibling feature: [`docs/specs/001-adapter-bootstrap/spec.md`](../001-adapter-bootstrap/spec.md)
  — Tier-1 SPI extension lands there
- Sibling feature (Tier-3 SPI):
  [`docs/specs/004-jdbc-connection-source/spec.md`](../004-jdbc-connection-source/spec.md)
