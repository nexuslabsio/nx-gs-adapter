...
- `CharacterClassDbDtoTest` — new, alongside the existing `CharacterSubclassDbDtoTest`: the old DTO is
  deprecated in this release, not deleted, so its guarantees still need a gate. Same round-trip
...
- `CharacterRuntimeDto` test — new fields round-trip; `hash(...)` changes when `classId` / `level` / `sp`
  ...
  nx-gameservers' `SyncIngestionIntegrationTest`. The hash one matters most: adding a column there
  silently invalidates every tenant's CRC snapshot and triggers a full resync storm, so it is called out
  in the provider's own code comments rather than left to a test that cannot exist yet.
