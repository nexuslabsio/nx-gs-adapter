...
- a **down-channel** command pair (`BanCommand` / `UnbanCommand`) on the
  existing `NxCommands` rail (see [`commands`](../commands/spec.md)) so the
...
> - [`commands`](../commands/spec.md) — Kafka commands topic + consumer +
>   dispatch table + reply path + heartbeat slot. UNCHANGED by this slice.
> - [`db-sync`](../db-sync/spec.md) — CDC engine + `DbSchemaProvider` SPI +
>   per-entity sync topics carrying the up-channel `BanDbDto`. UNCHANGED.
> - [`adapter-modules`](../adapter-modules/spec.md) — Tier-1 ServiceLoader
...
- Sibling reference (concrete-command handler + DTO migration pattern):
  [`docs/features/commands-send-mail/spec.md`](../commands-send-mail/spec.md)
- Commands rail infrastructure: [`docs/features/commands/spec.md`](../commands/spec.md)
- Up-channel sync infrastructure: [`docs/features/db-sync/spec.md`](../db-sync/spec.md)
